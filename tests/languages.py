...
for hu, en in builder.PAIRS.items():
    ...
    if hu != 'aszf.html':
        ...
        assert [builder.LOCALIZED_ASSETS.get(src, src) for src in documents[0].xpath('//img/@src')] == documents[1].xpath('//img/@src'), en
...
for contact, privacy in [('contact.html', 'adatkezeles.html'), ('contact-en.html', 'privacy-en.html')]:
    ...
    assert doc.xpath('//details[@id="booking-privacy"]//a[@href=$target]', target=privacy), contact

...
2026-09-25 price list: added a matching English-only second page to the root PDF; original bilingual first page verified unchanged. Both pages visually checked. New document temporary files belong in project-root `tmp/`, not V19. See PRINT_ASSETS.md.

2026-09-25 price-list footer follow-up: the highlighted bottom-left footer now displays only `mihaly.bence.fitness@gmail.com` in gold, with a matching email link. The former website footer and duplicate right contact are removed. Root PDF regenerated and visually checked.

2026-09-25 document update: recreated the owner's bilingual A4 price-list reference as root `mihaly-bence-prices-a4-hu-en.pdf`, with the approved business email in both visible text and mailto link. All reference prices/layout retained and final page visually verified. See PRINT_ASSETS.md. Website and older snapshots unchanged.
...
Added restrained, interruptible chart sweep/ratio transitions, result-card feedback and selection hover/colour transitions in HU/EN. Reduced motion skips effects; changing that setting cancels running effects. Values remain immediate and formulas/data handling are unchanged. Macro CSS/JS now v19.2. Verified with tests/macro-motion.cjs, calculation/structure/language checks and HU/EN browser interactions. See MACRO_CALCULATOR.md and MOTION_NOTES.md. Latest clean export: ../output/V19-public-macro-motion/ (98 files), superseding the dashboard export. Not deployed.
...
Completed 2026-09-23. V19 was created from the reviewed V18; all 186 copied files were verified byte-for-byte before V19-specific edits. V19 is the active working folder. Preserve V18 and all older versions.

## What is done

- Bronze/copper/gold/black design, HU/EN pages, prices, gallery, MB logo and existing interactions are preserved.
- Cal.com replaces Calendly with the owner's three configured events. Time-first booking, service/package notes, direct-calendar fallback and email/copy routes remain. Personal details are entered in Cal after choosing a slot. The owner now confirms everything works, including the dark outer canvas. Earlier in-app-preview failure notes are historical.
...
Build: tools/build-terms.py regenerates terms and invokes the language/privacy/error builders; tools/build-languages.py regenerates bilingual pages. For new normal-page copy, append inventory IDs with tools/extract-translations.py and supply English translations. Do not renumber IDs. Cache versions: fonts/section-nav CSS 18.6, script 19.3, styles 19.3, config 18.3, responsive 18.2.

Live V18 publication was owner-confirmed previously; this review's later edits have not been published. Preview 8180 serves the V18 snapshot; use http://127.0.0.1:8190/ for V19.

## Clean publication output

The V19-only `tools/build-public.py` prepares a fresh allowlisted upload directory. Current output: `../output/V19-public-instant-booking/` (90 files, 3,707,547 bytes). It includes all 22 public pages, their referenced assets, four public policy downloads, both font licences, CNAME and .nojekyll. It excludes Markdown, tests, tools, temporary files, print assets and unreferenced source photos. The output is not deployed. Rebuild to a new output directory after public-source changes; the tool deliberately refuses an existing directory. Remove the already-public internal Markdown from the actual deployment separately.
